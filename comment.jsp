...
<!-- banner -->
<% String hi="";
                    if(request.getParameter("comsub")!=null && request.getParameter("pid")!=null)
                    {
                        String user4 = "root";
                            String pass4 = "";
                            ...
                            {
                            try{
                                String q="select * from rusers where  rno=? ";
            ...
            if(myresult.next())
            {
                hi = myresult.getString("dept");
                               ...
                               try
                                {
                                    String qc="insert into commenttable(postid,deptid,type,comment,time,userid) values(?,?,?,?,?,?)";
                                       PreparedStatement mystatement41=myconnection.prepareStatement(qc);
                                       mystatement41.setInt(1, Integer.parseInt(request.getParameter("pid")));
                                        mystatement41.setString(2,hi );
                                       mystatement41.setString(3, "student");
                                       mystatement41.setString(4, request.getParameter("comment"));

                                        java.util.Date dat=new java.util.Date();
                                       mystatement41.setString(5, dat.toString());

                                       mystatement41.setString(6, session.getAttribute("rno").toString());
                                    if(mystatement41.executeUpdate()>0)
                                    {   
                                        
                                        response.sendRedirect("comment.jsp?pid="+ request.getParameter("pid"));
                                    }
                                }
                                catch(Exception e)
                                {
                            ...
                            {
                                out.print("Please Write Comment");
                            }
                    }
                %>
<div class="wthree_agile_admin_info">
        ...
        <!--top three starts here-->                                                                                
            ...
            String place4 = "community";
                            ...
                            myconnection=DriverManager.getConnection(path4+place4,user4,pass4);
                            try{
                                String qc = "delete from adminstudentnotificationtable where studentid=? and postid=?";
                                PreparedStatement my = myconnection.prepareStatement(qc);
                                my.setString(1, session.getAttribute("rno").toString());
                                my.setString(2, request.getParameter("pid"));
                                if(my.executeUpdate()>0)
                                {
...
%>    
            ...
            myconnection=DriverManager.getConnection(path4+place4,user4,pass4);
            try{
                                String qc = "delete from  teacherstudentnotificationtable where studentid=? and postid=?";
                                PreparedStatement my = myconnection.prepareStatement(qc);
                                my.setString(1, session.getAttribute("rno").toString());
                                my.setString(2, request.getParameter("pid"));
                                if(my.executeUpdate()>0)
...
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">    
  ...
  <div class="w3-row">
    ...
    try{
        ...
        try
        {
            String q="select * from posttable where postid=?";
            PreparedStatement mystatement=myconnection.prepareStatement(q);   
...
<%
    ...
    String user4 = "root";
          ...
          try
            {
                String q="select * from commenttable where postid=? order by commentid ";
                PreparedStatement mystatement=myconnection.prepareStatement(q);     
                mystatement.setString(1, request.getParameter("pid"));
                ResultSet myresult = mystatement.executeQuery();
                if(myresult.next())
                { out.print("<h4 class='midu'>Comments</h4>");
                    do
                    {
                        try{
                                         ...
                                         + "<span class='w3-right w3-opacity'>"+myresult.getString("time")+"</span>");
                                                out.print( "<h4>"+myresult3.getString("name")+" (Teacher)</h4><br>");
                                                out.print(myresult3.getString("deptname")+"<br><hr class='w3-clear'>");
                                            }
                                         }
                                         else 
                                         {
                                         ...
                                         + "<span class='w3-right w3-opacity'>"+myresult.getString("time")+"</span>");
                                          ...
                                          out.print("<p>"+myresult.getString("comment")+"</p>");
                                             
                                           if(myresult.getString("userid").equals(session.getAttribute("rno")))
                                           out.print("<a class='mylink' id='" + myresult.getString("commentid") + "' href='delcat.jsp'><button type='button' class='w3-button w3-theme-d1 w3-margin-bottom'><i class=' fa fa-trash' saria-hidden='true'></i> &nbsp;Delete</button></a> " );       
                           ...
                           {
                            out.print("Error in q2"+e.getMessage());
                           }                               
                    }while(myresult.next());
                    out.print("<h3 class='tittle-w3l'>"
                            + "<span class='heading-style'>"
        ...
        <div class="w3-col m12">
          <div class="w3-card w3-round w3-theme-d4">
            <div class="w3-container w3-padding">
                <form method="post">
              <textarea rows="0" cols="75" placeholder="Comment Here"contenteditable="true" name="comment" class="w3-border w3-padding w3-white"></textarea>
              <br>
                <input type="submit" name ="comsub" value="Post" class='w3-button w3-theme'>
